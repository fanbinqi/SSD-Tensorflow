...
def tf_summary_image(image, bboxes, name='image', unwhitened=False):
    ...
    #将图像写入summary，可以在tensorboard上进行可视化
...
def preprocess_for_train(image, labels, bboxes,
    ...
    with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
        ...
        if image.dtype != tf.float32:
            image = tf.image.convert_image_dtype(image, dtype=tf.float32)   #tf.image.draw_bounding_boxes要求图像矩阵中的数字为实数
        tf_summary_image(image, bboxes, 'image_with_bboxes')                # 利用tf.image.convert_image_dtype将图像矩阵转化为实数
