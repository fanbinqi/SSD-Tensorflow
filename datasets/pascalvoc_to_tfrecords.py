...
def run(dataset_dir, output_dir, name='voc_train', shuffling=False):
    ...
    """
    if not tf.gfile.Exists(dataset_dir):   #判断路径是否存在
        tf.gfile.MakeDirs(dataset_dir)    #创建一个目录

    # Dataset filenames, and shuffling.
    path = os.path.join(dataset_dir, DIRECTORY_ANNOTATIONS)
    filenames = sorted(os.listdir(path))   #sorted是排序函数
    if shuffling:
    ...
    while i < len(filenames):
        ...
        with tf.python_io.TFRecordWriter(tf_filename) as tfrecord_writer:    #tf.python_io模块是tensorflow用来处理tfrecords文件的接口，TFRecordWriter是将记录写入TFRecords文件的类
            j = 0
