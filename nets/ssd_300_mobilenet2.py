...
class SSDNet(object):
    ...
    default_params = SSDParams(
        ...
        feat_layers=['block6', 'block7', 'block8', 'block9', 'block10', 'block11'],
        feat_shapes=[(1, 1), (3, 3), (5, 5), (10, 10), (20, 20), (40, 40)],
        ...
        anchor_ratios=[[2, .5],
                       ...
                       [2, .5, 3, 1./3],
                       [2, .5]],
        anchor_steps=[300, 100, 60, 30, 15, 8],
        anchor_offset=0.5,
...
def ssd_anchor_one_layer(img_shape,
    ...
    #[[[0.013,0.013.....0.013]][[0.04.....0.04]]....[[7.98....7.98]]]
...
def ssd_net(inputs,
    ...
    def batch_norm(x):
        ...
        return x
    with tf.variable_scope(scope, 'ssd_300_mobilenet2', [inputs], reuse=reuse):
        #net = vgg.vgg_16(inputs)
        ...
        net = mobilenetv2.mobilenet_v2(inputs)
        print(net)

        end_point = 'block6'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 256, [3, 3], scope='conv6')
            net1 = slim.max_pool2d(net, [2, 2], 2, scope='pool-6-1')
            net1 = slim.conv2d(net1, 256, [1, 1], scope='conv1x1-1')
            net1 = slim.max_pool2d(net1, [2, 2], 2, scope='pool-6-2')
            net1 = slim.conv2d(net1, 256, [1, 1], scope='conv1x1-2')
            net1 = slim.max_pool2d(net1, [2, 2], 2, padding='VALID', scope='pool-6-3')
        end_points[end_point] = net1
        print("the first detection layer :", net1)

        # net1 = sub_pixel.PS(net, 2)
        # print(net1)
        end_point = 'block7'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 256, [3, 3], scope='conv7')
            net2 = slim.max_pool2d(net, [2, 2], 2, scope='pool-7-1')
            net2 = slim.conv2d(net2, 256, [1, 1], scope='conv1x1')
            net2 = slim.max_pool2d(net2, [2, 2], 2, scope='pool-7-2')
        end_points[end_point] = net2
        print("the second detection layer :", net2)

        end_point = 'block8'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 256, [3, 3], scope='conv8')
            net3 = slim.max_pool2d(net, [2, 2], 2, scope='pool8')
        end_points[end_point] = net3
        print("the third detection layer :", net3)
        ...
        print("the fourth detection layer :", net4)

        end_point = 'block10'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 512, [3, 3], scope='conv10')
            net5 = sub_pixel.PS(net, 2)
        end_points[end_point] = net5
        print("the fifth detection layer :", net5)

        end_point = 'block11'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 512, [3, 3], scope='conv11')
            net6 = sub_pixel.PS(net, 4)
        end_points[end_point] = net6
        print("the sixth detection layer :", net6)
